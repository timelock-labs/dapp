	...
	return (
		<div className='relative'>
			<Select.Root defaultValue={defaultValue}>
				<Select.Trigger aria-label={label} className={clsx('rounded p-2 transition-colors hover:bg-slate-200', isPending && 'pointer-events-none opacity-60')}>
					<Select.Icon>
						<LanguageIcon className='h-6 w-6 text-slate-600 transition-colors group-hover:text-slate-900' />
					</Select.Icon>
				</Select.Trigger>
				<Select.Portal>
					<Select.Content align='end' className='min-w-[8rem] overflow-hidden rounded bg-white py-1' position='popper'>
						<Select.Viewport>
							{items.map(item => (
