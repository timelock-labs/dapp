...
export const useTimelockImport = (config: TimelockImportConfig = {}) => {
   ...
   */
  const detectTimelockStandard = useCallback(async (
   ...
   */
  const getCompoundParameters = useCallback(async (
    contractAddress: Address
  ): Promise<Partial<TimelockParameters>> => {
    if (!provider) {
      throw new Error('Provider not available');
    }
    ...
    try {
      ...
      const [admin, pendingAdmin, delay, gracePeriod, minDelay, maxDelay] = await Promise.race([
        parametersPromise,
        timeoutPromise,
      ]) as [string, string, ethers.BigNumber, ethers.BigNumber, ethers.BigNumber, ethers.BigNumber];
   ...
   */
  const getOpenZeppelinParameters = useCallback(async (
    contractAddress: Address
  ): Promise<Partial<TimelockParameters>> => {
   ...
   */
  const fetchTimelockParameters = useCallback(async (
    contractAddress: Address
   ...
   */
  const validateContractAddress = useCallback(async (address: Address): Promise<boolean> => {
    try {
