...
import type { HttpMethod, FilterParams, PaginationParams } from '@/types';
...
export function useCrudOperations<TEntity, TCreateRequest, TUpdateRequest, TFilters extends FilterParams = FilterParams>(
	...
	// Mutations
	const createMutation = useApiMutation<TEntity, TCreateRequest>(baseEndpoint, 'POST', {
	...
	const useEntityList = (filters: TFilters = {} as TFilters) => {
		return useFilteredApi<TEntity[], TFilters>(baseEndpoint, filters, {
			autoFetch: true,
	...
	const update = useCallback(
		async (id: string | number, data: TUpdateRequest) => {
 ...
 */
export function useDataExport(endpoint: string, entityName = 'data') {
	...
	>(endpoint, 'POST', { defaultErrorMessage: `Failed to export ${entityName}` });

	const exportData = useCallback(
		async (format: 'csv' | 'xlsx' | 'json', filters?: Record<string, unknown>) => {
