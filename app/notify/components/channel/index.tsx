...
export default function Channel() {
	...
	const fetchChannels = useCallback(async () => {
		try {
			...
			if (response && response.data) {
				const data = [
					response.data.feishu_configs.map((item: Channel) => ({
						...item,
						channel: 'feishu',
					})),
	...
	return (
		<div>
			...
			<AddChannelModal isOpen={isAddChannelModalOpen} onClose={() => setIsAddChannelModalOpen(false)} onSuccess={handleAddChannelSuccess} />
		</div>
	);
}
