...
interface Transaction {
  ...
  transactions: string;
  createdAt: string;
  type: 'Pendding' | 'Witting' | 'Badge'; // Types based on image
  endTime: string;
...
const PendingTransactions: React.FC = () => {
  // Updated getBadgeStyle to match new requirements
  const getBadgeStyle = (type: Transaction['type']) => {
    switch (type) {
      case 'Pendding':
        return 'bg-white text-gray-800 border border-gray-300'; // White background, gray text, gray border
      case 'Witting':
      case 'Badge':
        return 'bg-black text-white border border-black'; // Black background, white text, black border
      default:
        return 'bg-gray-100 text-gray-800 border border-gray-300'; // Default fallback
    }
  };
    ...
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 flex flex-col h-full">
      ...
      <div className="flex-grow overflow-x-auto overflow-y-auto">
        <table className="min-w-full divide-y divide-gray-200">
          {/* Table Header - background color changed to light gray */}
          ...
          <tbody className="bg-white divide-y divide-gray-100">
            {dummyTransactions.map((tx) => (
              <tr key={tx.id}>
                {/* Chain content styling: light gray background, rounded corners */}
                <td className="px-2 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                  <div className="inline-flex items-center space-x-1 bg-gray-100 rounded-full px-2 py-1">
                    {tx.chainIcon}
                    <span className="text-gray-800">{tx.chain}</span> {/* Ensure chain text color is correct */}
                  </div>
                </td>
                ...
                {/* Type content styling: conditional background and border */}
