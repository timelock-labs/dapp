          ...
          <div className="flex flex-wrap gap-2">
            {mailboxOptions.map((option) => (
              <div key={option.id} className="flex items-center space-x-2 py-2 px-4 rounded-md hover:bg-gray-50 transition-colors bg-[#F5F5F5] h-8">
                <Checkbox id={String(option.id)} checked={selectedMailbox.includes(option.email)} onCheckedChange={(checked) => handleCheckboxChange(option.email, checked as boolean)} />
