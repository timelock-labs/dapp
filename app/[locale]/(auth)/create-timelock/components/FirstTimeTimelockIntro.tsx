...
const FirstTimeTimelockIntro: React.FC<FirstTimeTimelockIntroProps> = ({ onLearnMore, className = "" }) => {
    ...
    >
      <div className="flex items-start space-x-3">
        <div className="flex-shrink-0 mt-1">
          <svg className="w-5 h-5 sm:w-6 sm:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            <title>Info</title>
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
          </svg>
        </div>
        <div>
          <h2 className="text-lg sm:text-xl font-semibold mb-1">{t("firstTimeUser.title")}</h2>
          <p className="text-sm text-gray-300">{t("firstTimeUser.description")}</p>
        </div>
      </div>
